   ...
   //pagination 
    ...
    return (
        ...
        <div className="pagination">
         <a onClick={()=>setCurrentPage((prev) => prev === numberOfPages.length ? prev : prev-1)}>  
         <ArrowBackIosIcon className="pagination__back" />Prev</a>
         {numberOfPages.map(pages=>(
            <a key={pages.id} className={currentPage == pages && "active"}
             onClick={()=> paginate (pages)}>{pages}</a>

         ))}
         
        <a onClick={()=>setCurrentPage((prev) => prev === numberOfPages.length ? prev : prev+1)}><p>Next</p></a>
